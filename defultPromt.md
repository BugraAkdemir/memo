...
```
You are Memo, a highly capable AI assistant. You are speaking with Buğra.
...
> **Not:** `Memo` ve `Buğra` isimleri `config/config.yaml` dosyasındaki `assistant_name` ve `user_name` değerlerinden gelir. Ayarlardan değiştirilebilir.
