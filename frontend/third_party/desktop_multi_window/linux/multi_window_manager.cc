...
std::string MultiWindowManager::Create(FlValue* args) {
  ...
  // MEMO PATCH: request an alpha-capable visual so a sub-window that asks
  // for a translucent background (window_manager's setBackgroundColor,
  // called from the sub-window's own Dart isolate once it's running) can
  // actually render one. This MUST happen before gtk_widget_realize()
  // below — GTK only honours a visual change made pre-realize, and this
  // function is the only place that runs before that, since the plugin's
  // own window-created callback (the documented, unpatched extension
  // point) fires after realize. See frontend/third_party/README.md.
  GdkScreen* memo_rgba_screen = gtk_window_get_screen(window);
  GdkVisual* memo_rgba_visual = gdk_screen_get_rgba_visual(memo_rgba_screen);
  if (memo_rgba_visual != nullptr &&
      gdk_screen_is_composited(memo_rgba_screen)) {
    gtk_widget_set_visual(GTK_WIDGET(window), memo_rgba_visual);
  }

  // MEMO PATCH: this vendored copy is only ever used for the desktop
  // mascot's window today, which is undecorated and tiny from the start —
  // so skip building (and then immediately tearing down again, via the
  // sub-window's own Dart-side setAsFrameless() call) the upstream code's
  // GtkHeaderBar, which was sized for its original 1280x720 default. That
  // header bar's own minimum-width layout constraint is what left the
  // actual GTK surface stuck at its original size on this Wayland session
  // even after window_manager's later setSize() calls reported success
  // (getSize() agreed with the new size; the compositor kept showing the
  // old one regardless) — creating the window at its real, final, small
  // size and never decorating it at all sidesteps that renegotiation
  // entirely. If this file is ever reused for a bigger, chrome-ful
  // sub-window, bring back upstream's GNOME-header-bar-detection block
  // instead of widening this one.
  // set_decorated(FALSE) alone left a KWin/Wayland server-side titlebar
  // in place (confirmed live) — GTK only suppresses that when the app has
  // gone through gtk_window_set_titlebar at least once, which is what
  // triggers the client-side-decoration negotiation with the compositor.
  // An empty, invisible one still counts, and renders as nothing.
  ...
  // gtk_window_set_keep_above() is an X11 EWMH mechanism
  // (_NET_WM_STATE_ABOVE) with no native-Wayland equivalent in plain GTK
  // — confirmed live that alone, it did nothing on this machine's
  // GDK_BACKEND=wayland KWin/Plasma session (another window still covered
  // the mascot once focused). Wayland compositors deliberately refuse an
  // unprompted "raise me above everything" request from a client, the
  // same security model that blocks unprompted focus-stealing. Rather
  // than chase KDE's own compositor-specific protocol
  // (org_kde_plasma_window_management), linux/runner/main.cc forces the
  // whole app onto XWayland (setenv GDK_BACKEND=x11, before GTK reads
  // it) — on X11 this same call works exactly as intended, confirmed
  // live by opening another window directly over the mascot's screen
  // position and screenshotting: the mascot stayed on top.
