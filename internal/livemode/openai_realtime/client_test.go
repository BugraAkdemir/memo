...
// and the test waits for the real, already-running readLoop goroutine to
// actually observe the read error and close its channels itself — with
// ctx still live throughout, matching a genuine network drop rather than a
// Close()/cancellation. Before the fix, runToolCall's
// `select { case c.events <- ev: case <-c.ctx.Done(): }` had only one
// ready case here (ctx.Done() never fires) — the closed-channel send —
// guaranteeing a panic, not just a possible one.
func TestClient_RunToolCallAfterReadLoopExitDoesNotPanic(t *testing.T) {
	...
	c := NewClient("oa-key", "gpt-realtime-2.1", "sys", nil, func(context.Context, string, json.RawMessage) (string, error) {
		return "result", nil
	})
	...
	func() {
		...
		c.runToolCall(serverEvent{Type: serverEventFunctionCallArgsDone, CallID: "call-1", Name: "test_tool", Arguments: "{}"})
	}()
}
