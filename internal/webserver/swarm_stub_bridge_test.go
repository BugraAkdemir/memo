...
func (b *swarmStubBridge) HostSwarmCreate(modelPath string) (string, error) {
	if b.hostCreate != nil { return b.hostCreate(modelPath) }
	return "", nil
}
func (b *swarmStubBridge) HostSwarmAddWorker(id, secret, myRPCAddress, label string) error {
	if b.addWorker != nil { return b.addWorker(id, secret, myRPCAddress, label) }
	return nil
}
func (b *swarmStubBridge) HostSwarmRemoveWorker(id string) error {
	if b.removeWorker != nil { return b.removeWorker(id) }
	return nil
}
func (b *swarmStubBridge) HostSwarmReorderWorkers(fromIdx, toIdx int) error {
	if b.reorder != nil { return b.reorder(fromIdx, toIdx) }
	return nil
}
func (b *swarmStubBridge) HostSwarmSetShare(id string, pct float64) error {
	if b.setShare != nil { return b.setShare(id, pct) }
	return nil
}
func (b *swarmStubBridge) HostSwarmStart(ctxSize int) error {
	if b.start != nil { return b.start(ctxSize) }
	return nil
}
func (b *swarmStubBridge) HostSwarmStop() error {
	if b.stop != nil { return b.stop() }
	return nil
}
func (b *swarmStubBridge) HostSwarmClose() error {
	if b.close != nil { return b.close() }
	return nil
}
func (b *swarmStubBridge) HostSwarmStatus() interface{} {
	if b.hostStatus != nil { return b.hostStatus() }
	return map[string]string{"role": "none"}
}
func (b *swarmStubBridge) JoinSwarm(code string) error {
	if b.join != nil { return b.join(code) }
	return nil
}
func (b *swarmStubBridge) LeaveSwarm() error {
	if b.leave != nil { return b.leave() }
	return nil
}
func (b *swarmStubBridge) JoinSwarmStatus() interface{} {
	if b.joinStatus != nil { return b.joinStatus() }
	return map[string]string{"role": "none"}
}
func (b *swarmStubBridge) SwarmStatusSnapshot() interface{} {
	if b.status != nil { return b.status() }
	return map[string]string{"role": "none"}
}

// ─── FullBridge no-ops (everything not covered above) ───
func (b *swarmStubBridge) SendMessageStream(ctx context.Context, userMsg string) <-chan api.StreamChunk { ch := make(chan api.StreamChunk); close(ch); return ch }
func (b *swarmStubBridge) SendMessageWithImageStream(ctx context.Context, userMsg string, imagePath string) <-chan api.StreamChunk { return b.SendMessageStream(ctx, userMsg) }
func (b *swarmStubBridge) SendMessageWithFileStream(ctx context.Context, userMsg string, filePath string) <-chan api.StreamChunk { return b.SendMessageStream(ctx, userMsg) }
...
func (b *swarmStubBridge) GetMinimalModeOverrides() (bool, bool, bool, bool) { return false, false, false, false }
...
func (b *swarmStubBridge) ImportMemoryFromText(ctx context.Context, rawText string) (int, bool, error) { return 0, false, nil }
func (b *swarmStubBridge) GenerateSelfInsight(ctx context.Context, windowDays int, lang string) (string, error) { return "", nil }
...
func (b *swarmStubBridge) FilteredMemorySearch(query string, topK int, since string, tag string) []memory.MemoryResult { return nil }
...
func (b *swarmStubBridge) SearchModels(query string) ([]modelstore.HFModelResult, error) { return nil, nil }
func (b *swarmStubBridge) GetModelFiles(repoID string) []modelstore.GGUFFile { return nil }
func (b *swarmStubBridge) DownloadModel(repoID, filename string, expectedSize int64) error { return nil }
...
func (b *swarmStubBridge) StartLocalModel(modelPath string, ctxSize, port, gpuLayers int) error { return nil }
...
func (b *swarmStubBridge) SetTailscaleMode(enabled bool, authKey, hostname string, funnel bool, port int) error { return nil }
...
func (b *swarmStubBridge) DevGatewayChatStream(ctx context.Context, modelSpec string, req provider.ChatRequest) (<-chan provider.StreamChunk, string, error) {
	ch := make(chan provider.StreamChunk); close(ch); return ch, "", nil
}
...
func (b *swarmStubBridge) GetOrchestraConfig() orchestra.OrchestraConfig { return orchestra.OrchestraConfig{} }
func (b *swarmStubBridge) UpdateOrchestraConfig(cfg orchestra.OrchestraConfig) error { return nil }
func (b *swarmStubBridge) GetProactiveSettings() config.ProactiveConfig { return config.ProactiveConfig{} }
...
func (b *swarmStubBridge) UpdateSyncSettings(enabled bool, clientID, clientSecret, passphrase, tokenPath string, intervalMessages int) error { return nil }
...
func (b *swarmStubBridge) WhatsAppSend(ctx context.Context, jid, text string) (string, error) { return "", nil }
func (b *swarmStubBridge) WhatsAppSearch(query string, limit int) ([]whatsapp.Message, error) { return nil, nil }
func (b *swarmStubBridge) WhatsAppGetChats() ([]whatsapp.ChatSummary, error) { return nil, nil }
func (b *swarmStubBridge) WhatsAppGetMessages(chatJID string, limit int) ([]whatsapp.Message, error) { return nil, nil }
...
func (b *swarmStubBridge) WhatsAppChatStream(ctx context.Context, userMsg string) <-chan api.StreamChunk { return b.SendMessageStream(ctx, userMsg) }
...
func (b *swarmStubBridge) CreateTaskList(chatID, title string, items []string) (*taskloop.TaskList, error) { return nil, nil }
