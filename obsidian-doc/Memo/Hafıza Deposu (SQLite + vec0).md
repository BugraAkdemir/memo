...
Memo, hafıza depolaması için **SQLite** veritabanını `sqlite-vec` eklentisiyle birlikte kullanır.
...
SQLite'ın yerleşik işlem desteği sayesinde yazma sırasında oluşabilecek hatalar tüm veritabanını bozmaz. Ya tüm işlem başarılı olur ya da hiçbir değişiklik kalıcı olmaz.

### 2. Eşzamanlı Erişim (Concurrent Access)
SQLite, WAL (Write-Ahead Logging) modunda okuma ve yazma işlemlerinin eşzamanlı çalışmasına izin verir.

### 3. Artımlı Güncelleme
Yeni mesaj kaydederken sadece **INSERT** sorgusu çalışır. Tüm veritabanını yeniden yazmak gerekmez.

### 4. Lazy Loading Gerekmez
Veriler diskte yapılandırılmış şekilde durur. Sorgular sadece gerekli kayıtları getirir.
...
2. Bu metin embedding modeline gönderilir → `[0.12, -0.55, ...]` vektörü
3. `vec0` ANN indeksi üzerinden yaklaşık en yakın komşu (ANN) araması
...
| Hafızayı sıfırlayabilir miyim? | `data/memory/memo.db` dosyasını silerek |
