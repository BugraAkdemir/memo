#!/usr/bin/env bash
# Memo BETA — one-line installer / updater for Linux / macOS.
#
#   curl -fsSL https://download.bugradev.com/get-memo-beta.sh | bash
#
...
echo -e "${NC}"
echo -e "${BOLD}  Local-first, privacy-focused AI assistant ${YELLOW}(BETA)${NC}"
...
os="$(uname -s)"
case "$os" in
    Linux)  url="$DOMAIN/memo_beta.tar.gz" ;;
    Darwin) url="$DOMAIN/memo-mac_beta.zip" ;;
    *)
